  ...
  return (
    <div className="container-wide py-8 md:py-12">
      <nav className="flex items-center gap-2 text-sm text-[var(--color-text-muted)] mb-6 flex-wrap">
        ...
        <ChevronRight className="w-3 h-3" />
        <span className="text-[var(--color-text-primary)] font-medium">{id}</span>
      </nav>

      <div className="flex items-center justify-between mb-8">
        <div>
          <h1 className="text-2xl font-bold font-[var(--font-heading)]">Order {id}</h1>
          <p className="text-sm text-[var(--color-text-muted)]">Placed on Apr 15, 2026</p>
        </div>
      ...
      <div className="mb-8 p-6 rounded-xl border border-[var(--color-border)] bg-[var(--color-surface-elevated)]">
            ...
            <div key={step.label} className="flex-1 flex items-center">
              <div className="flex flex-col items-center text-center flex-1">
                <div className={`w-10 h-10 rounded-full flex items-center justify-center mb-2 ${
                  step.done ? "bg-[var(--color-success)] text-white" : "bg-[var(--color-surface-muted)] text-[var(--color-text-muted)]"
                }`}>
        ...
        <div className="mt-4 pt-4 border-t border-[var(--color-border)]">
          <p className="text-sm"><strong>Tracking:</strong> DTDC123456789</p>
          <a href="#" className="text-sm text-[var(--color-accent)] hover:underline">Track on carrier website →</a>
        </div>
      </div>
      ...
      <div className="grid lg:grid-cols-3 gap-8">
        <div className="lg:col-span-2">
          ...
          <div className="space-y-3">
            {[
              { title: "Midnight Bloom Abstract", size: "A3", qty: 1, price: 449 },
              { title: "Geodesic Pen Holder", size: "Standard", qty: 2, price: 549 },
            ].map((item) => (
              <div key={item.title} className="flex gap-4 p-4 rounded-xl border border-[var(--color-border)]">
                <div className="w-16 h-16 rounded-lg bg-[var(--color-surface-muted)] shrink-0" />
                <div className="flex-1">
                  <p className="font-medium text-sm">{item.title}</p>
                  <p className="text-xs text-[var(--color-text-muted)]">Size: {item.size} · Qty: {item.qty}</p>
                </div>
        ...
        <div>
          ...
          <div className="p-5 rounded-xl border border-[var(--color-border)] space-y-3">
            ...
            <div className="pt-3 border-t border-[var(--color-border)]">
              <p className="text-xs text-[var(--color-text-muted)]">Payment: Prepaid (UPI)</p>
              <p className="text-xs text-[var(--color-success)]">✓ Payment confirmed</p>
            </div>
          </div>
          ...
          <div className="p-5 rounded-xl border border-[var(--color-border)] text-sm text-[var(--color-text-secondary)]">
            <p className="font-medium text-[var(--color-text-primary)]">PrintMacha Customer</p>
            <p>123 Example Street, Koramangala</p>
